...
class Post_Generator:
    ...
    def __handlePath(self, line: str, indent_level=4) -> str:
        ...
        print(file_path)
        self.lines.append(
            f'{self.id_list[indent_level + 4]}span.code-file-path {file_path}')
        ...
        if len(code_line) == 0:
            ...
            self.lines.append(
                f'{self.id_list[indent_level + 5]}{line[idx_end + l_end:]}')
            self.lines.append(
                f'{self.id_list[indent_level + 4]}br')
        return code_line
    ...
    def __handleString(self, line: str, indent_level=4) -> str:
        ...
        if code_line[:l_start] == ':string-open ':
            ...
            self.lines.append(
                f'{self.id_list[indent_level + 4]}span.code-string {string}')
            code_line = line[idx_end + l_end:]
            print(code_line)
        return code_line
    ...
    def __handleBrackets(self, line: str, indent_level=4) -> str:
        ...
        if code_line[:l_start] == ':bracket-open ':
            ...
            print(open_bracket)
            self.lines.append(
                f'{self.id_list[indent_level + 4]}span.code-bracket {open_bracket}')
            ...
            self.lines.append(
                f'{self.id_list[indent_level + 4]}span.code-bracket {closing_bracket}')
    ...
    def __addCodeBlock(self, code_content: dict, indent_level=4):
        ...
        for line in code_content["content"]:
            ...
            while ':code-specific ' in code_line or ':path ' in code_line or ':bracket-open ' in code_line or ':string-open ' in code_line:
                ...
                elif code_line[:len(':path-start ')] == ':path-start ':
